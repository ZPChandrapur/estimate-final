...
const ItemMeasurements: React.FC<ItemMeasurementsProps> = ({
 ...
 const handleAddMeasurement = async () => {debugger
  ...
  try {
    ...
    const { data: rateData, error: rateFetchError } = await db
      .schema('estimate')
      .from('item_rates')
    ...
    const { data: measurementsForRate, error: measurementsError } = await db
      .schema('estimate')
      .from('item_measurements')
    ...
    const { error: updateRateError } = await db
      .schema('estimate')
      .from('item_rates')
      .update({
  ...
  const updateItemSSRQuantity = async () => {
    try {
      ...
      const { error: updateError } = await createFreshClient()
        .schema('estimate')
        .from('subwork_items')
        .update({
